...
export default function PlanetarySystem({ projects, onProjectSelect, onProjectHover }) {
  ...
  // Animation progress for each planet's rings
  const ringAnimationsRef = useRef({})

  // Generate textures only once when component mounts
  useEffect(() => {
    // Pre-generate all textures at once
    projects.forEach((project, index) => {
      ...
      // Add some swirls or patterns based on project index
  ...
  // Memoize planet data to prevent unnecessary recalculations
  ...
  useFrame((_, delta) => {
    // Rotate the entire system slowly if not hovering
    if (groupRef.current && hoveredPlanet === null) {
      groupRef.current.rotation.y += delta * 0.05
    }

    // Update all planet materials based on hover state
